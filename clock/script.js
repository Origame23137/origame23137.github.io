...
/* =========================================================
   MODULE 1bis — MÉTÉO (Open-Meteo, sans clé API)
   =========================================================
   Géocodage du nom de ville -> coordonnées via l'API Geocoding
   d'Open-Meteo, puis récupération de la météo actuelle via l'API
   Forecast. Aucune clé requise, tout se fait côté navigateur.
   ========================================================= */

const WEATHER_CITY_KEY = "dashboard.weather.city";
const WEATHER_REFRESH_MS = 15 * 60 * 1000; // 15 min : suffisant, la météo actuelle bouge peu
...
const weatherCitySaveBtn = document.getElementById("weather-city-save");
const weatherStatusEl = document.getElementById("weather-status");

// Cache en mémoire pour éviter de re-géocoder à chaque rafraîchissement
// si la ville n'a pas changé.
let weatherCoordsCache = null;

// Codes météo WMO (utilisés par Open-Meteo) -> description en français.
// Référence : https://open-meteo.com/en/docs (WMO Weather interpretation codes)
const WMO_DESCRIPTIONS = {
    0: { day: "Grand soleil", night: "Ciel dégagé" },
    1: { day: "Plutôt ensoleillé", night: "Plutôt dégagé" },
    2: { day: "Partiellement nuageux", night: "Partiellement nuageux" },
    3: { day: "Ciel couvert", night: "Ciel couvert" },
    45: { day: "Brouillard", night: "Brouillard" },
    48: { day: "Brouillard givrant", night: "Brouillard givrant" },
    51: { day: "Bruine légère", night: "Bruine légère" },
    53: { day: "Bruine", night: "Bruine" },
    55: { day: "Bruine forte", night: "Bruine forte" },
    56: { day: "Bruine verglaçante", night: "Bruine verglaçante" },
    57: { day: "Bruine verglaçante", night: "Bruine verglaçante" },
    61: { day: "Pluie légère", night: "Pluie légère" },
    63: { day: "Pluie", night: "Pluie" },
    65: { day: "Forte pluie", night: "Forte pluie" },
    66: { day: "Pluie verglaçante", night: "Pluie verglaçante" },
    67: { day: "Pluie verglaçante", night: "Pluie verglaçante" },
    71: { day: "Neige légère", night: "Neige légère" },
    73: { day: "Neige", night: "Neige" },
    75: { day: "Forte neige", night: "Forte neige" },
    77: { day: "Neige en grains", night: "Neige en grains" },
    80: { day: "Averses", night: "Averses" },
    81: { day: "Averses", night: "Averses" },
    82: { day: "Fortes averses", night: "Fortes averses" },
    85: { day: "Averses de neige", night: "Averses de neige" },
    86: { day: "Fortes averses de neige", night: "Fortes averses de neige" },
    95: { day: "Orage", night: "Orage" },
    96: { day: "Orage avec grêle", night: "Orage avec grêle" },
    99: { day: "Orage avec grêle", night: "Orage avec grêle" },
};

function getWeatherDescription(code, isDay) {
    const entry = WMO_DESCRIPTIONS[code];
    if (!entry) return "Météo indisponible";
    return isDay ? entry.day : entry.night;
}

function setWeatherLine(text) {
    weatherLineEl.textContent = text;
}

function setWeatherStatus(message) {
    weatherStatusEl.textContent = message;
    clearTimeout(setWeatherStatus._timer);
    setWeatherStatus._timer = setTimeout(() => {
        weatherStatusEl.textContent = "";
    }, 3000);
}

// Convertit un nom de ville en coordonnées (latitude/longitude).
...
async function updateWeather() {
    ...
    if (!city) {
        setWeatherLine("Configure ta ville dans les réglages");
        return;
    }

    try {
        // On ne re-géocode que si la ville a changé, pour économiser des
        // requêtes inutiles à chaque rafraîchissement.
        if (!weatherCoordsCache || weatherCoordsCache.city !== city) {
        ...
        const current = await fetchCurrentWeather(
            weatherCoordsCache.latitude,
            weatherCoordsCache.longitude
        );

        const description = getWeatherDescription(current.weather_code, current.is_day === 1);
        const temperature = Math.round(current.temperature_2m);

        setWeatherLine(`${weatherCoordsCache.displayName}, ${description.toLowerCase()}, ${temperature}°C`);
    } catch (err) {
        console.warn("Impossible de récupérer la météo :", err);
        setWeatherLine("Météo indisponible");
    }
}
...
function saveWeatherCity() {
    ...
    if (!city) {
        setWeatherStatus("Indique un nom de ville.");
        return;
    }

    localStorage.setItem(WEATHER_CITY_KEY, city);
    weatherCoordsCache = null; // force un nouveau géocodage
    setWeatherStatus("Ville enregistrée.");
    updateWeather();
}

weatherCitySaveBtn.addEventListener("click", saveWeatherCity);
weatherCityInput.addEventListener("keydown", (event) => {
    if (event.key === "Enter") saveWeatherCity();
});

function initWeather() {
    const savedCity = localStorage.getItem(WEATHER_CITY_KEY);
    if (savedCity) weatherCityInput.value = savedCity;
...
const SPOTIFY_SCOPES = [
    ...
    "user-modify-playback-state", // requis pour play/pause/next/previous
];
...
async function redirectToSpotifyAuth() {
    if (!SPOTIFY_CLIENT_ID || SPOTIFY_CLIENT_ID === "REMPLACE_PAR_TON_CLIENT_ID") {
        alert(
...
async function exchangeCodeForToken(code) {
    ...
    if (!response.ok) throw new Error("Échec de l'échange du code d'autorisation.");
...
function disconnectSpotify() {
    [LS_ACCESS_TOKEN, LS_REFRESH_TOKEN, LS_EXPIRES_AT, LS_PKCE_VERIFIER].forEach((k) =>
...
function updateSpotifyConnectButton() {
    ...
    spotifyConnectLabel.textContent = connected ? "Déconnecter Spotify" : "Connecter Spotify";
}
...
async function fetchCurrentlyPlaying() {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Erreur API Spotify : ${response.status}`);

        const data = await response.json();

        // Pas de morceau chargé du tout (rare hors du cas 204 déjà géré
        // au-dessus) : on affiche l'état "aucune musique".
        if (!data || !data.item) {
            setIdleState();
            return;
        }

        // Un morceau est chargé mais en pause : on garde son titre, sa
        // pochette et sa progression affichés, avec l'icône "play" —
        // au lieu de basculer sur l'état "aucune musique en écoute".
...
/* ---------- Contrôle de la lecture (play / pause / next / previous) ---------- */
